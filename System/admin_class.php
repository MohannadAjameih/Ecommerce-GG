...
class Action
{
	...
	function login()
	{
		...
		$qry = $this->db->query("SELECT *, CONCAT(firstname, ' ', lastname) AS name FROM users WHERE email = '" . $email . "' AND password = '" . md5($password) . "' AND type <> 3");
	...
	function update_product()
	{
		...
		$data = "";
		foreach ($_POST as $k => $v) {
			if (!in_array($k, array('id', 'product_name', 'old_price', 'new_price', 'Discount', 'image','Offer_date')) && !is_numeric($k)) {
